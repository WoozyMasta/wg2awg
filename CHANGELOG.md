...
* Morph Mode with per-slot handshake obfuscation derived from shared `MorphKey`,
  including config/env loading, `-g` key generation,
  and `-P` probe CLI with `-S` slot override.
  Inbound handshakes accept the current slot +/-2 (5 slots total),
  guaranteeing at least +/-240s of clock-skew tolerance.
* wg2awg-only `[Interface].ObfsProfile` config support.
...
* `obfs dns_like`: extra trailing bytes were appended to the wrapped packet
  but not tracked, so unwrap returned a payload that was too long;
  WireGuard rejected the oversized handshake.
